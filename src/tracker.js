import { X11Display } from "../x11/display.js";
...
export class MouseTracker {
  ...
  async start() {
    ...
    try {
      ...
      // Start polling for mouse position and button changes
      this.tracking = true;
      this.poll();
    ...
    while (this.tracking) {
      try {
        ...
        if (pos.dx !== 0 || pos.dy !== 0) {
          if (process.env.DEBUG) {
            console.debug(`${gray}Broadcasting mouse_move: dx=${pos.dx}, dy=${pos.dy} to ${this.peer.peers.size} peers`);
          }
          ...
          if (changed) {
            ...
            if (pressed) {
              ...
              if (button === "left" && this.dragging) {
                if (process.env.DEBUG) {
                  console.debug(`${gray}Broadcasting mouse_drag: dx=${pos.x - this.dragStartX}, dy=${pos.y - this.dragStartY}`);
                }
